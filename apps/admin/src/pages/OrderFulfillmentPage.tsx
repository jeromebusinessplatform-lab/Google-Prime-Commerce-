...
import { ChevronLeft, CheckCircle2, X, ShieldAlert, ShieldCheck, AlertCircle, Printer } from 'lucide-react';
...
export function OrderFulfillmentPage() {
  ...
  useEffect(() => {
    loadOrder().catch(error => setNotice(error.message));
  }, [id]);
  ...
  const isReview = status === 'payment_review' || status === 'PENDING' || status === 'PAYMENT_FAILED' || status.startsWith('HOLD_') || status === 'REQUEST_RESUBMIT' || status === 'FINAL_FOLLOWUP';
  ...
  const reviewActions = useMemo(() => {
    if (status === 'payment_review' || status === 'PENDING') return [
    ...
    <div className="bg-gray-50 dark:bg-gray-950 flex-1 text-gray-900 dark:text-gray-100">
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 p-4 sticky top-0 z-20 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/orders')} className="p-2"><ChevronLeft size={22} /></button>
          <div><h2 className="text-lg font-bold uppercase tracking-tight">Order {order.id}</h2><div className="text-[10px] uppercase tracking-widest text-gray-400">Order Management</div></div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
            <div className="flex items-center justify-between mb-5">
              <div><h3 className="text-[11px] font-black uppercase tracking-widest">Order Management</h3><p className="text-[10px] text-gray-400 uppercase tracking-widest mt-1">Dynamic workflow controls</p></div>
              <div className="flex gap-2">
                <button onClick={() => setMode('REVIEW')} className={`px-4 py-2 rounded-lg text-[10px] font-black tracking-widest ${mode === 'REVIEW' ? 'bg-black text-white' : 'border border-gray-200 dark:border-gray-700'}`}>REVIEW</button>
                <button onClick={() => setMode('MODIFY')} className={`px-4 py-2 rounded-lg text-[10px] font-black tracking-widest ${mode === 'MODIFY' ? 'bg-black text-white' : 'border border-gray-200 dark:border-gray-700'}`}>MODIFY</button>
              </div>
            </div>
            ...
            {mode === 'REVIEW' && !isReview && nextFulfillment && (
              <button disabled={busy} onClick={() => statusAction(nextFulfillment)} className="w-full py-4 bg-black text-white rounded-xl text-[11px] font-black uppercase tracking-[0.18em]">{label(nextFulfillment)}</button>
            )}
            ...
            {mode === 'MODIFY' && (
              <div className="space-y-3">
                <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-3 text-[10px] uppercase tracking-widest">UPDATED → PAYMENT CONFIRMED → START PACKING → READY → AWAITING RIDER → DISPATCHED → DELIVERED</div>
                <textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="Modification note" className="w-full min-h-24 rounded-lg border border-gray-200 dark:border-gray-700 bg-transparent p-3 text-sm" />
                <button disabled={busy} onClick={amendment} className="w-full py-3 bg-black text-white rounded-xl text-[10px] font-black uppercase tracking-widest">UPDATED</button>
              </div>
            )}

            <div className="mt-5 pt-5 border-t border-gray-100 dark:border-gray-800">
              <div className="text-[9px] uppercase tracking-widest text-gray-400 mb-3">Fulfillment Progress</div>
              <div className="flex items-center gap-1 overflow-x-auto">
                {FULFILLMENT.map((stage, index) => <React.Fragment key={stage}><div className={`shrink-0 px-2 py-1 rounded text-[8px] font-bold uppercase tracking-widest ${currentIndex >= index ? 'bg-black text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-400'}`}>{label(stage)}</div>{index < FULFILLMENT.length - 1 && <div className={`h-px w-5 shrink-0 ${currentIndex > index ? 'bg-black' : 'bg-gray-200'}`} />}</React.Fragment>)}
              </div>
            </div>
          </div>

          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
            <h3 className="text-[10px] font-black uppercase tracking-widest mb-4">Order Items</h3>
            <div className="divide-y divide-gray-100 dark:divide-gray-800">{(order.items || []).map((item: any) => <div key={item.id} className="py-3 flex justify-between text-xs"><span>{item.name} × {item.quantity || item.qty || 1}</span><span>₱{Number(item.price || 0).toLocaleString()}</span></div>)}</div>
            <div className="pt-4 mt-2 border-t border-gray-200 dark:border-gray-800 flex justify-between font-bold">TOTAL <span>₱{Number(order.total || 0).toLocaleString()}</span></div>
          </div>
        </div>

        <div className="space-y-4">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
            <h3 className="text-[10px] font-black uppercase tracking-widest mb-4">Payment</h3>
            <div className="text-xs space-y-2"><div className="flex justify-between"><span className="text-gray-400">METHOD</span><span>{order.payment?.method || '—'}</span></div><div className="flex justify-between"><span className="text-gray-400">STATUS</span><span>{paymentStatus}</span></div><div className="flex justify-between"><span className="text-gray-400">REVIEW</span><span>{order.reviewStatus || '—'}</span></div></div>
            {order.receipt?.imageUrl && <img src={order.receipt.imageUrl} alt="Payment proof" className="mt-4 w-full rounded-lg border border-gray-200 dark:border-gray-700" />}
          </div>
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
            <h3 className="text-[10px] font-black uppercase tracking-widest mb-4">Customer</h3>
            <div className="text-sm">{order.customerName || 'Walk-in'}</div><div className="text-xs text-gray-500 mt-1">{order.customerPhone || '—'}</div><div className="text-xs text-gray-500 mt-3">{order.address || '—'}</div>
          </div>
