  ...
  return (
    <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl flex flex-col max-h-[90vh]">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar Tabs */}
        <div className="w-48 bg-gray-50 border-r border-gray-200 overflow-y-auto p-2 space-y-1">
          {tabs.map(tab => (
            <button
              key={tab.id}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-black outline-none min-h-[100px]"
                  >
                    {categories.map(cat => (
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button 
                        ...
                        className="px-2 bg-gray-100 border border-gray-300 rounded hover:bg-gray-200"
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-4">
                  {formData.media.map((item: Media, index: number) => (
                    <div key={item.id} className="relative group border border-gray-200 rounded-lg overflow-hidden aspect-square bg-gray-50 shadow-sm">
                      <img src={item.url} alt={item.altText} className="w-full h-full object-cover" />
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        <button type="button" className="p-2 bg-white rounded-full text-gray-800 hover:text-red-500 shadow-lg"><Trash2 size={16} /></button>
                      </div>
                      <div className="absolute top-2 left-2 bg-black/60 text-white text-[10px] px-1.5 py-0.5 rounded backdrop-blur-sm">#{index + 1}</div>
                    </div>
                  ))}
                  <div className="border-2 border-dashed border-gray-200 rounded-lg aspect-square flex flex-col items-center justify-center text-gray-400 hover:border-black hover:text-black transition-all cursor-pointer bg-gray-50">
                    <input 
                      type="file" 
